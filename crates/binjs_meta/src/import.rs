...
impl Importer {
    ...
    /// let name_lazy = builder.get_node_name("LazyFoo")
    ///     .expect("Missing name LazyFoo");
    ///
    /// {
    ///     let interface_eager = builder.get_interface(&name_eager)
    ///         .expect("Missing interface EagerFoo");
    ///     let value_field = /* interface_eager[value] */;
    ///     assert_eq!(value_field.is_lazy(), false);
    /// }
    ///
    /// {
    ///     let interface_lazy = builder.get_interface(&name_lazy)
    ///         .expect("Missing interface LazyFoo");
    ///     let value_field = /* interface_lazy[value] */;
    ///     assert_eq!(value_field.is_lazy(), false);
    /// }
    /// ```
    ...
    fn import_interface(&mut self, interface: &Interface) {
        ...
        for member in &interface.members {
            if let InterfaceMember::Attribute(Attribute::Regular(ref attribute)) = *member {
                ...
                fields.push((name, type_, is_lazy));
            } else {
        ...
        let mut node = self.builder.add_interface(&name)
            .expect("Name already present");
        for (field_name, field_type, field_is_lazy) in fields.drain(..) {
            node.with_field(&field_name, field_type, field_is_lazy);
        }
    }
