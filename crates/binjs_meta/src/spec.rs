...
impl Obj {
    ...
    fn with_field_aux(self, name: &FieldName, type_: Type, is_lazy: bool, doc: Option<&str>) -> Self {
    ...
    pub fn with_field(self, name: &FieldName, type_: Type, is_lazy: bool) -> Self {
        self.with_field_aux(name, type_, is_lazy, None)
    }

    pub fn with_field_doc(self, name: &FieldName, type_: Type, is_lazy: bool, doc: &str) -> Self {
        self.with_field_aux(name, type_, is_lazy, Some(doc))
    }
}
...
impl InterfaceDeclaration {
    ...
    pub fn with_field(&mut self, name: &FieldName, type_: Type, is_lazy: bool) -> &mut Self {
        self.with_field_aux(name, type_, is_lazy, None)
    }
    pub fn with_field_doc(&mut self, name: &FieldName, type_: Type, is_lazy: bool, doc: &str) -> &mut Self {
        self.with_field_aux(name, type_, is_lazy, Some(doc))
    }
    fn with_field_aux(&mut self, name: &FieldName, type_: Type, is_lazy: bool, doc: Option<&str>) -> &mut Self {
        let mut contents = Obj::new();
        std::mem::swap(&mut self.contents, &mut contents);
        self.contents = contents.with_field_aux(name, type_, is_lazy, doc);
        self
    }
}
